...
@SpringBootTest(properties = {
        ...
        "app.discovery.gnews.api-key=test-api-key",
        "spring.datasource.url=jdbc:h2:mem:gnews-discovery-startup;DB_CLOSE_DELAY=-1",
        "spring.datasource.driver-class-name=org.h2.Driver",
        "spring.datasource.username=sa",
        "spring.datasource.password=",
        "spring.jpa.hibernate.ddl-auto=create-drop"
})
class GNewsDiscoveryEnabledStartupTest {

    @Autowired
    private ApplicationContext applicationContext;

    @Test
    void selectedGNewsStartsWithOneProviderAndDiscoveryService() {
        assertThat(applicationContext.getBeansOfType(NewsDiscoveryProvider.class))
                .hasSize(1)
                .allSatisfy((name, provider) ->
                        assertThat(provider).isInstanceOf(GNewsNewsDiscoveryProvider.class)
                );
